...
pub async fn start_server(
    ...
    // Start WebSocket producer — wire it to the *shared* broadcast channel
    // in ServerState so that `recolor_and_broadcast()` feeds connected clients.
    let ws_producer = WsProducer::from_sender(config.ws_addr, state.ws_tx.clone());
