...
impl WsProducer {
    ...
    /// Create a producer that reuses an *existing* broadcast sender.
    /// This is the preferred constructor when the sender is already
    /// stored in `ServerState::ws_tx`, so that both `recolor_and_broadcast`
    /// and the WebSocket listener share the exact same channel.
    pub fn from_sender(addr: SocketAddr, tx: broadcast::Sender<StreamFrame>) -> Self {
        Self {
            addr,
            broadcast_tx: tx,
        }
